...
class Media {
  ...
  constructor({
    ...
    this.font = font;
    this.url = url; // Store the optional URL
    this.createShader();
    this.createMesh();
    this.createTitle();
    this.onResize();
  ...
  createShader() {
    ...
    void main() {
      ...
      // Rounded corners for landscape cards
      float d = roundedBoxSDF(vUv - 0.5, vec2(0.5 - uBorderRadius * 1.5), uBorderRadius * 1.5);
      if(d > 0.0) {
        discard;
      }
  ...
  onResize({
    ...
    this.scale = this.screen.height / 1000; // Adjusted base scale
    this.plane.scale.y =
      (this.viewport.height * (400 * this.scale)) / this.screen.height;
    this.plane.scale.x =
      (this.viewport.width * (700 * this.scale)) / this.screen.width; // Wider
...
class App {
  ...
  createMedias(
    ...
    this.medias = this.mediasImages.map((data, index) => {
      return new Media({
        ...
        borderRadius,
        font,
        url: data.url, // Pass the optional URL
      });
    });
  }
  ...
  addEventListeners() {
    ...
    this.boundOnWheel = (e) => {
      e.preventDefault(); // ⛔️ Stop page scroll
      this.onWheel(e);
    };
  ...
  onMouseMove(e: MouseEvent) {
    ...
    for (const media of this.medias) {
      ...
      const width = plane.scale.x;
      const height = plane.scale.y;
      ...
      if (
        x > ndcX - width / this.viewport.width &&
        x < ndcX + width / this.viewport.width &&
        y > ndcY - height / this.viewport.height &&
        y < ndcY + height / this.viewport.height
      ) {
        // Mouse is over this card
        this.hoveredMedia = media;

        // Calculate local mouse position (-0.5 to 0.5)
        const localX = ((x - ndcX) * this.viewport.width) / width;
        const localY = ((y - ndcY) * this.viewport.height) / height;

        // Max tilt in radians (e.g., 0.25 ~ 14deg)
        const maxTilt = 0.25;
        media.plane.rotation.x = -localY * maxTilt;
        media.plane.rotation.y = localX * maxTilt;
        found = true;
      } else if (this.hoveredMedia === media) {
        // Reset tilt if not hovered anymore
        media.plane.rotation.x = 0;
        media.plane.rotation.y = 0;
      }
    }
    if (!found && this.hoveredMedia) {
      this.hoveredMedia.plane.rotation.x = 0;
      this.hoveredMedia.plane.rotation.y = 0;
